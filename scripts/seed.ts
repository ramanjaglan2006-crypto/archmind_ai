...
async function seed() {
    ...
    const toInsert = architecturePatterns.filter((p) => !existingTitles.has(p.title));

    if (toInsert.length === 0) {
        console.log("All patterns already exist. Nothing to seed.");
        return;
    }

    console.log(`Seeding ${toInsert.length} architecture patterns...`);

    for (const pattern of toInsert) {
        const combinedText = `${pattern.title}: ${pattern.description}`;
        console.log(`  Generating embedding for "${pattern.title}"...`);

        const embedding = await generateEmbedding(genAI, combinedText);

        const { error } = await supabase.from("architecture_patterns").insert({
            title: pattern.title,
        ...
        if (error) {
            console.error(`  Failed to insert "${pattern.title}":`, error.message);
        } else {
            console.log(`  Inserted "${pattern.title}" (${embedding.length} dimensions)`);
        }
